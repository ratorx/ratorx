...
CONTENT_JS := $(call ts_to_js,$(CONTENT_TS_SRC)) node_modules/content
WEB_TS_SRC := $(call find_ts,web)
WEB_JS := $(call ts_to_js,$(WEB_TS_SRC)) node_modules/web
UTILS_TS_SRC := $(call find_ts,utils)
UTILS_JS := $(call ts_to_js,$(UTILS_TS_SRC)) node_modules/utils
...
all: web;
web: $(HTML) $(CSS) $(JS);
install: $(addprefix node_modules/,$(WORKSPACES));
...
node_modules/%: | $(NPM_MARKER)
	ln -sfTv ../$(TS_OUT_DIR)/$* $@

# Find tsc, or use npx
$(TS_MARKER) : $(CONTENT_TS_SRC) $(WEB_TS_SRC) $(UTILS_TS_SRC) tsconfig.json $(NPM_MARKER)
	npx tsc
	@touch $(TS_MARKER)
